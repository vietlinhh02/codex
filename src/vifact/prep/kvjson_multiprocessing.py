...
# Standalone function for multiprocessing - must be at module level  
def process_claims_batch(batch_data):
    """Process a batch of claims for evidence retrieval"""
    claims_batch, id_to_chunks_subset, bm25_tokens_data, doc_info, topk_bm25 = batch_data
...
def prepare_from_kvjson_multiprocessing(
    in_path: str,
    out_dir: str,
    ...
    """
    True multiprocessing version with proper pickle-safe functions
    """
    ...
    print(f"[prepare_multiprocessing] Starting at {time.strftime('%H:%M:%S')}")
    print(f"[prepare_multiprocessing] Using {n_workers} processes (CPU limit: 6)")
    
    with open(in_path, "r", encoding="utf-8") as f:
        data: Dict[str, Dict] = json.load(f)
    total_items = len(data)
    print(f"[prepare_multiprocessing] Loaded {total_items} items")
    print(f"[prepare_multiprocessing] Config: max_tokens={max_tokens_per_chunk}, topk_bm25={topk_bm25}")

    # Step 1: Process documents in parallel
    print(f"[prepare_multiprocessing] Step 1/4: Processing {total_items} documents with {n_workers} processes...")
    ...
    print(f"[prepare_multiprocessing] Created {len(batches)} batches of ~{batch_size} documents each")
    ...
    with ProcessPoolExecutor(max_workers=n_workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            print(f"[prepare_multiprocessing] Batch {completed}/{len(batches)} completed - {elapsed:.1f}s elapsed")
    
    all_doc_ids = [did for did, _ in corpus_rows]
    all_texts = [text for _, text in corpus_rows]
    
    step1_time = time.time() - step1_start
    print(f"[prepare_multiprocessing] Step 1 completed in {step1_time:.1f}s - Created {len(corpus_rows)} chunks")

    # Save corpus
    print(f"[prepare_multiprocessing] Saving corpus to CSV...")
    ...
    print(f"[prepare_multiprocessing] Corpus saved: {len(corpus_rows)} chunks")

    # Step 2: Build BM25
    print(f"[prepare_multiprocessing] Step 2/4: Building BM25 index...")
    step2_start = time.time()
    
    print(f"[prepare_multiprocessing] Tokenizing {len(all_texts)} chunks...")
    bm25_corpus_tokens = []
    for i, text in enumerate(all_texts):
        tokens = tokenize_bm25(text)
        bm25_corpus_tokens.append(tokens)
        if (i + 1) % 5000 == 0 or (i + 1) == len(all_texts):
            print(f"[prepare_multiprocessing] Tokenized {i+1}/{len(all_texts)} ({(i+1)/len(all_texts)*100:.1f}%)")
    
    print(f"[prepare_multiprocessing] Creating BM25 index...")
    bm25 = BM25Okapi(bm25_corpus_tokens)
    step2_time = time.time() - step2_start
    print(f"[prepare_multiprocessing] Step 2 completed in {step2_time:.1f}s")

    # Step 3: Process claims in parallel
    print(f"[prepare_multiprocessing] Step 3/4: Processing {total_items} claims with multiprocessing...")
    step3_start = time.time()
    
    claims_items = list(enumerate(data.items(), start=1))
    claim_batch_size = max(20, len(claims_items) // (n_workers * 2))
    
    # Prepare batches for claims processing
    claim_batches = []
    for i in range(0, len(claims_items), claim_batch_size):
        batch = claims_items[i:i + claim_batch_size]
    ...
    print(f"[prepare_multiprocessing] Created {len(claim_batches)} claim batches")
    
    records = []
    with ProcessPoolExecutor(max_workers=n_workers) as executor:
        futures = {executor.submit(process_claims_batch, batch): i for i, batch in enumerate(claim_batches)}
        ...
        for future in as_completed(futures):
            ...
            print(f"[prepare_multiprocessing] Claim batch {processed_batches}/{len(claim_batches)} - "
                  f"Claims: {processed_claims}/{total_items} ({processed_claims/total_items*100:.1f}%) - "
                  f"{rate:.1f} claims/sec - ETA: {eta:.0f}s")

    step3_time = time.time() - step3_start
    print(f"[prepare_multiprocessing] Step 3 completed in {step3_time:.1f}s - Processed {len(records)} claims")

    # Step 4: Save outputs
    print(f"[prepare_multiprocessing] Step 4/4: Saving outputs...")
    ...
    print(f"[prepare_multiprocessing] Writing {len(train_recs)} train records...")
    write_jsonl(train_jsonl, train_recs)
    print(f"[prepare_multiprocessing] Writing {len(valid_recs)} validation records...")
    write_jsonl(valid_jsonl, valid_recs)

    # Save bm25.json
    print(f"[prepare_multiprocessing] Saving BM25 mappings...")
    ...
    print(f"[prepare_multiprocessing] Step 4 completed in {step4_time:.1f}s")
    print(f"[prepare_multiprocessing] 🎉 ALL DONE! Total time: {total_time:.1f}s ({total_time/60:.1f} minutes)")
    print(f"[prepare_multiprocessing] Wrote outputs to {out}")
